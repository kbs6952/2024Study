...
namespace Class12
{
    ...
    // 람다식의 기본 문법
    // 매개 변수 => 식
    // "=>" 입력 연산자 : 왼쪽의 매개 변수을 오른쪽 식에 전달하는 연산자입니다.

    // "식", "문"
    // 식(expression) : 한 줄로 표현한 코드.(중괄호를 없애기위함)
    ...
    {
        Caculate cal1 = delegate (int a, int b) { return a + b; };
    ...
    // delegate에서 익명 메소드에 대한 문법을 지원해줄 때 C#2.0 람다식을 지원x
    // 람다식을 지원해주기 시작하였는데,
    // 기존의 기능을 완벽히 대체하는 새로운 기능을 구현을 했는데, 그럼에도 불구하고 기존의 기능을 살려두는 이유가 무엇일까?
    ...
    // 사전준비가 필요하기 때문에 이 문법을 사용을 할때 실수를 할 가능성이 생긴다.

    // C#에서 제공해주는 기본 대리자
    // 1. Func 대리자 : 매개 변수를 17개 가지는 오버로딩으로 반환 값이 있는 대리자 형식입니다.
    ...
    class SampleClass
    {
        ...
        // 3. 메소드
        public void ListAdd(string addString)
        {
            list.Add(addString);
        }
        public void ListAdd2(string addString)=>list.Add(name);
